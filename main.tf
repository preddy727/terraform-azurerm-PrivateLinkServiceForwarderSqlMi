resource "azurerm_resource_group" "pls_forwarder" {
...
data "template_file" "cloudinit" {
  template = file("${path.root}/cloudconfig.tpl")
  vars = {
...
module "vmss-cloudinit" {
  source                                 = "./Modules/terraform-azurerm-vmss-cloudinit"
...
resource "azurerm_private_link_service" "pls" {
  name                = var.pl_service
  location            = azurerm_resource_group.pls_forwarder.location
  resource_group_name = azurerm_resource_group.pls_forwarder.name
